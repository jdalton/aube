...
test('the repo Dockerfile prebake (when present) matches the tracked pins', t => {
  ...
  // The repo deliberately ships no rust-toolchain.toml (#1039); the
  // prebake's toolchain assertion no-ops on an empty channel.
  const toolchainAbs = path.join(REPO_ROOT, SURFACES.toolchainToml)
  const toolchain = existsSync(toolchainAbs) ? readFileSync(toolchainAbs, 'utf8') : ''
  ...
  // With no tracked toolchain file the drift is synthesized: a channel
  // the image cannot have pre-installed must be flagged.
  const drifted = toolchain
    ? toolchain.replace(/channel = ".*"/, 'channel = "nightly-1999-01-01"')
    : 'channel = "nightly-1999-01-01"\n'
  assert.ok(checkDockerPrebake(docker, tools, drifted).length > 0)
})
