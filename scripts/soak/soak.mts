...
import { PPA_RUST_CEILING, REPO_ROOT, SURFACES } from './paths.mts'
...
/**
 * MSRV ceiling — the #1039 guard. The Ubuntu PPA builds aube's source
 * package on Launchpad with the DISTRO rustc; the first soak landing
 * (#1020) bumped the workspace MSRV past it (1.95 > 1.93) and the whole
 * port was reverted to unbreak the release. Every `rust-version` in the
 * given Cargo.toml must stay <= PPA_RUST_CEILING (paths.mts, with the
 * dated "checked:" note for when Launchpad moves).
 */
export function checkMsrvCeiling(body: string, file: string): Finding[] {
  if (!PPA_RUST_CEILING) {
    return []
  }
  const ceiling = parseRustVersion(PPA_RUST_CEILING)
  // Compare at the ceiling's precision: a "1.93" ceiling admits any
  // 1.93.x patch release (Launchpad ships patch bumps freely).
  const precision = PPA_RUST_CEILING.split('.').length
  const out: Finding[] = []
  for (const m of body.matchAll(/^\s*rust-version\s*=\s*"([^"]+)"/gm)) {
    const found = parseRustVersion(m[1]!)
    if (!found) {
      out.push({
        file,
        what: 'msrv parse',
        saw: m[1]!,
        wanted: 'a plain X.Y[.Z] rust-version',
        fix: 'use a plain version so the PPA ceiling stays checkable',
      })
      continue
    }
    if (compareVersions(found.slice(0, precision), ceiling!.slice(0, precision)) > 0) {
      out.push({
        file,
        what: 'msrv above the Ubuntu PPA rustc ceiling',
        saw: `rust-version = "${m[1]}"`,
        wanted: `<= ${PPA_RUST_CEILING} (Launchpad's rustc — see paths.mts PPA_RUST_CEILING)`,
        fix: 'keep the MSRV at the distro rustc, or raise the ceiling in paths.mts ONLY after Launchpad ships a newer rustc (this exact bump forced the #1039 revert)',
      })
    }
  }
  return out
...
function parseRustVersion(v: string): number[] | null {
...
export function main(argv: string[] = process.argv.slice(2)): number {
  ...
  const surfaces: Array<{
    ...
    // The #1039 guard: workspace MSRV must not outrun Launchpad's rustc.
    ...(PPA_RUST_CEILING ? [{ rel: SURFACES.cargoToml, check: checkMsrvCeiling }] : []),
  ]
