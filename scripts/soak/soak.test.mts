...
test('msrv ceiling: rust-version above the PPA rustc is a finding (#1039 guard)', () => {
  // Ceiling is 1.93 (paths.mts PPA_RUST_CEILING) — the exact bump that
  // forced the #1039 revert must be caught.
  assert.equal(checkMsrvCeiling('rust-version = "1.91"\n', 'c').length, 0)
  assert.equal(checkMsrvCeiling('rust-version = "1.93"\n', 'c').length, 0)
  assert.equal(checkMsrvCeiling('rust-version = "1.93.1"\n', 'c').length, 0)
  const findings = checkMsrvCeiling('rust-version = "1.95"\n', 'c')
  assert.equal(findings.length, 1)
  assert.match(findings[0]!.what, /PPA/)
  // Every rust-version line in the file is checked, not just the first.
  const multi = 'rust-version = "1.91"\n[package]\nrust-version = "1.96"\n'
  assert.equal(checkMsrvCeiling(multi, 'c').length, 1)
  // Unparseable pins are findings, not silent passes.
  assert.match(checkMsrvCeiling('rust-version = "^1.91"\n', 'c')[0]!.what, /parse/)
})
